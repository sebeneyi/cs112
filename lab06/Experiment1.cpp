 ...
 * Date: March 12, 2019
 * Edited by: Sebene Yi for CS 112 at Calvin College (lab06)
 */
...
/*
 * calculates the time to append to a list of a certain size
 * @param: unsigned size
 * return:
 * postcondition:
 */
void Experiment1::timeAppendToListOfSize(unsigned size){
	cout << " -- time to append to a full list of size "
		...
		myTimer.reset();
		for (int i = 0; i < REPS; i++) {               // repeatedly...
			list<int> l(size);                       // construct new size-n vector
			myTimer.start();                           // start timer
...
/*
 * calculates the time repeated to append until list is a certain size
 * @param: unsigned maxSize
 * return:
 * postcondition:
 */
